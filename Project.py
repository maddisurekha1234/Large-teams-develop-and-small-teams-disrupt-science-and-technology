...
def init_spark():
    print("creating spark session...gcp1-12G-e1x-run1")
...
def get_i_value(spark, df_repo_raw3, df_cc, df_focal):
    ...
    df_rearrage_cc_join = spark.sql(
        " SELECT e.cc_repo_id AS focal_repo, f.source_repo_citing_focal_repo, e.actor_id from df_cc_tbl e INNER JOIN df_rearrage_tbl f ON e.cc_repo_id != f.source_repo_citing_focal_repo")
    print("df_rearrage_cc_join schema")
    ##output 1
    ...
    # df_repo_raw3.createOrReplaceTempView("df_repo_raw3_tbl")
    df_focal_only_pull_events_join = spark.sql(
    ...
    ##JOIN OUTPUT 1 AND OUTPUT 2 on actor id
    df_rearrage_cc_join.createOrReplaceTempView("df_rearrage_cc_join_tbl")
    df_focal_only_pull_events_join.createOrReplaceTempView("df_focal_only_pull_events_join_tbl")

    df_join = spark.sql(
        " SELECT e.focal_repo_id, e.source_repo_citing_focal_repo, e.destination_repo_id  from df_rearrage_cc_join_tbl e INNER JOIN df_focal_only_pull_events_join_tbl f ON e.actor_id = f.actor_id")
    print("df_join show")
    df_join.show(10)
    return df_join
...
def main():
    ...
    # fdf = read_input(spark, "gs://project-work-aug/Output_Focal_Ref/focal_df", ",", "csv", "True")
    # print("count of focal: ", fdf.count())

    print("showing focal df")
    fdf.show(5)
    fdf.printSchema()
